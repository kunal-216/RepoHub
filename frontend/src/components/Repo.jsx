...
const Repo = ({repo}) => {
    ...
    const createdOn = formatMemberSince(repo?.created_at);
    ...
    return (
        <li className='mb-10 ms-7'>
            ...
            <div className='flex gap-2 items-center flex-wrap'>
                ...
                <span onClick={()=>handleCloneClick(repo)} className='cursor-pointer bg-green-100 text-green-800 text-xl font-medium px-2.5 py-0.5 rounded-full flex items-center gap-1'>
                    <FaCopy /> Clone
                </span>
            </div>

            <time className='block my-1 text-xs font-normal leading-none text-gray-400'>
                Released on {createdOn}
            </time>
            {repo.description? <p className='mb-4 text-base font-normal text-gray-500'>{repo.description.slice(0,500)}</p> : <p className='mb-4 text-base font-normal text-gray-500'>No description provided</p>}
